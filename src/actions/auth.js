...
const register = async (dispatch, errorDispatch, { name, surname, email, password }) => {
  UserAPI.count()
    .then((res) => {
      console.log(res);
    ...
    .catch((err) => {
      ErrorAPI.returnError(errorDispatch, err.response.data, err.response.status, "REGISTER_FAIL");
...
const login = async (dispatch, errorDispatch, { identifier, password }) => {
  ...
  axios
    ...
    .catch((err) => {
      ...
      ErrorAPI.returnError(errorDispatch, err.response.data, err.response.status, "LOGIN_FAIL");
