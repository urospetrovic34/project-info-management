...
const SystemAdminPage = () => {
  ...
  const users = userHooks.useUsersV2(null, debouncedUserName, debouncedPageNumber);
  ...
  useEffect(() => {
    if (users.data?.data?.length === 0 && users.data?.meta?.pagination.page > 1) {
      setPageNumber(0);
    }
  }, [users]);
      ...
      <div className={SystemAdminPageCSS.container}>
        ...
        <div className={SystemAdminPageCSS.flex}>
          <div className={SystemAdminPageCSS.search_container}>
            <div>
              <Input placeholder={"Search users..."} onChange={handleUserNameFilter} />
            </div>
            <div className={SystemAdminPageCSS.select}>
              <Select placeholder={"Sort by..."} options={sortEmployeeOptions} multi={false} isSearchable={false} />
            </div>
          </div>
        ...
        <div className={SystemAdminPageCSS.card_container}>
          {users.status === "success" && users.data?.data?.map((user) => <UserCard key={user.id} user={user} />)}
        </div>
