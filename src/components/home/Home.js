...
export const Home = () => {
    ...
    const projects = projectHooks.useProjects(debouncedProjectName,debouncedPageNumber);
        ...
        <div className={HomeCSS.wrapper}>
            ...
            <div className={HomeCSS.container}>
                {projects.status === "success" && (
                    <>
                        {projects.data.data.length === 0 && <div>
                            You don't have any projects
                        </div>}
                        <div className={HomeCSS.card_container}>
                            {projects.data?.data.map((project) => (
                                <Link key={project.id} to={`/projects/${project.id}`}><CardProjectInfo key={project.id} project={project}/></Link>
                            ))}
                        </div>
