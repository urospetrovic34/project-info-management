...
export const FileModal = (props) => {
  ...
  const handleArrowLeft = () => {
    const index = props.allFiles.findIndex((file) => file.id === currentFile.id);
    if (index > 0) {
  ...
  return (
    <div className={FileModalCSS.wrapper}>
      <div className={FileModalCSS.button_header_container}>
        <h3 className={FileModalCSS.headerContainer}>{props.note.attributes.title}</h3>
      ...
      {props.allFiles.findIndex((file) => file.id === currentFile.id) > 0 && props.allFiles.length > 1 ? (
        <div className={FileModalCSS.arrow_container}>
          <AiOutlineArrowLeft className={FileModalCSS.arrow} onClick={handleArrowLeft} />
        </div>
      ) : (
        ...
        {currentFile.attributes.ext === ".pdf" ? (
          ...
          <div className={FileModalCSS.video_contaienr}>
            <video width="1000" controls>
              <source src={currentFile.attributes.url} type={currentFile.attributes.mime} />
            </video>
          </div>
      ...
      {props.allFiles.findIndex((file) => file.id === currentFile.id) < props.allFiles.length - 1 &&
      props.allFiles.length > 1 ? (
        <div className={FileModalCSS.arrow_container}>
          <AiOutlineArrowRight className={FileModalCSS.arrow} onClick={handleArrowRight} />
        </div>
      ) : (
        <div className={FileModalCSS.arrow_container}></div>
      )}
