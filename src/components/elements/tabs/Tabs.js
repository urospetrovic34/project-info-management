...
const Tabs = (props) => {
  ...
  useEffect(() => {
    let array = props.project.data?.data.attributes.notes.data?.map((note) => {
  ...
  return (
    <div className={TabsCSS.tabs}>
        ...
        <div className={TabsCSS.categories_nav}>
          ...
          {props.project.status === "success" &&
            categories.map((category) => (
              <TabNavItem key={category} title={category} activeTab={activeTab} handleActiveTab={handleActiveTab} />
            ))}
        </div>
        {authState.user.role.name === "Project Manager" && (
          <div>
            <Link to={`/notes/create/${props.project.data?.data?.id}`}>
      ...
      <div className={TabsCSS.option_row}>
        <div>
          <Input placeholder="Search notes..." onChange={handleProjectNameFilter} />
        </div>
        ...
        <TabContent id={activeTab} activeTab={activeTab}>
          ...
          {props.project.status === "success" &&
            props.project.data?.data.attributes.notes.data
              .filter(
                (note) =>
                  note.attributes.category.data.attributes.name === activeTab &&
                  note.attributes.title.toLowerCase().includes(debouncedProjectName.toLowerCase())
              )
              .sort((a, b) => {
                if (sortEntry === "oldest") {
                  let aDate = new Date(a.attributes.createdAt);
                  let bDate = new Date(b.attributes.createdAt);
                  return aDate - bDate;
                  ...
                  return b.attributes.title.toLowerCase().localeCompare(a.attributes.title.toLowerCase());
                } else {
                  let aDate = new Date(a.attributes.createdAt);
                  let bDate = new Date(b.attributes.createdAt);
                  return bDate - aDate;
              ...
              .slice(9 * (debouncedCurrentPage - 1), 9 * debouncedCurrentPage)}
        </TabContent>
        <div className={TabsCSS.pagination}>
          {props.project.status === "success" && props.project.data?.data.attributes.notes.data && (
            <Pagination
